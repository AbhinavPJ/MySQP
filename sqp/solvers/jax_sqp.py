...
def kkt(P, q, A, b, active_mask, eps=1e-8): #kkt solver,given active set mask
    ...
    reg_p = 1e-8 #regularization for positive definiteness
    reg_d = 1e-8 #regularization for dual stability
    REG_MAX = 1e-2 #maximum regularization
    P_reg = P + reg_p * jnp.eye(n, dtype=P.dtype)  # add small multiple of identity to P to ensure P is positive definite
    A_masked = A * active_mask[:, None]   #apply active set mask to constraint matrix
    b_masked = b * active_mask #apply active set mask to constraint RHS
    lower_diag = (1.0 - active_mask) * 1.0 - active_mask * reg_d #dual regularization for inactive constraints
    KKT = jnp.block([
        [P_reg, A_masked.T],
        [A_masked, jnp.diag(lower_diag)]
    ]) #build KKT matrix
    rhs = jnp.concatenate([-q, b_masked])  #build KKT right-hand side
    sol_fast = jnp.linalg.solve(KKT, rhs)  #solve KKT system
    def solve_shifted(_):
        shift = 1e-6 #small diagonal shift for regularization
        KKT_shifted = KKT + shift * jnp.eye(n + m, dtype=KKT.dtype)
        return jnp.linalg.solve(KKT_shifted, rhs)
    is_bad = jnp.logical_or(jnp.any(jnp.isnan(sol_fast)), jnp.max(jnp.abs(sol_fast)) > 1e8) #check for bad solution
    sol = lax.cond(is_bad, solve_shifted, lambda _: sol_fast, operand=None) #resolve if needed
    p = sol[:n] #primal variables
    nu = sol[n:] #dual variables
...
@jax.jit
def update_hessian(B, s, y):  #better hessian update than earlier
...
def unconstrained_bfgs(x0, f, max_iter, tol, return_history=False):
    ...
    def step(carry, _):
        ...
        B_new = update_hessian(B, s, y) #update Hessian
...
def solve_sqp_fixed_iter(x0,f, c, ineq_indices,max_iter=100,tol=1e-8, eta=0.25, tau=0.5, rho=0.5, mu0=10.0, return_history=False, params=None):
    ...
    def sqp_step(carry, iter_num):
        x, lam, B, mu, conv_acc, act_iters, key = carry #carry: carry from previous iteration
        ...
        viol_mask = (c_k > -1e-8).astype(jnp.float64) #which constraints are violated
        mult_mask = (lam > 1e-8).astype(jnp.float64) #which multipliers are positive 
        ...
        mu_needed_lam = jnp.max(jnp.abs(lam_qp)) * 1.2
        ...
        return (jnp.where(conv_acc, x, x_new), 
                ...
                mu, final_conv, final_iter, key), x_new
    init = (x, lam, B, mu, False, max_iter, jax.random.PRNGKey(0))
    (xf, lf, Bf, _, cv, it, _), x_hist= lax.scan(sqp_step, init, jnp.arange(max_iter), length=max_iter)
    if return_history:
        x_hist = jnp.vstack([x0[None, :], x_hist])
        hist_len = jnp.where(cv, it + 1, max_iter + 1)
        x_hist = x_hist[:hist_len]
        f_hist = jax.vmap(f)(x_hist)
        return xf, lf, Bf, cv, x_hist, f_hist, it        
    return xf, lf, Bf, cv, it
def solve_sqp(x0,f,c,ineq_indices=None, max_iter=100, tol=1e-8, eta=0.25, tau=0.5, rho=0.5, mu0=10.0 ,cache_key=None) : #just a wrapper to match signature
...
def solve_sqp_diff(x0, params, f_fn, c_fn, ineq_indices, max_iter=100, tol=1e-8, eta=0.25, tau=0.5, rho=0.5): 
    def fwd(x0, params):
    ...
    def bwd(residuals, g_in):
        ...
        H = Bf + 1e-8 * jnp.eye(n) #regularized Hessian
        Am = J * active_mask[:, None] #all inactive constraints have zero rows
        KKT = jnp.block([
            [H, Am.T],
            [Am, jnp.diag((1.0 - active_mask))]
        ])#solve KKT system
